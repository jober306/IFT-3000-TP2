...
module Tp2h15 : TP2H15 = struct
  ...
  open Str
	...
	(* appartient : 'a -> 'a list -> bool Retourner si un élément existe ou  *)
	(* non dans une liste                                                    *)

  let appartient e l = exists (fun x -> x = e) l

	(* enlever : 'a -> 'a list -> 'a list Enlever un élément dans une liste *)

  let enlever e l = 
    let (l1, l2) = partition (fun x -> x = e) l
    in l2

	(* remplacer : 'a -> 'a -> 'a list -> 'a list Remplacer un élément par   *)
  ...
  let remplacer e e' l =
	...
	(* que des éléments uniques Les chaînes vides sont également enlevées de *)
	(* la liste ainsi que les espaces inutiles avant et/ou après les chaînes *)
  ...
  let uniques liste =
	...
	(* en découpant une chaîne selon un séparateur (p.ex "|")                *)

  let decouper_chaine chaine separateur = split (regexp separateur) chaine

	(* formater_chaine : string list -> string Construire une chaîne selon   *)
  ...
  let formater_chaine liste = 
	...
	(* float Retourne le nombre de secondes depuis l'année 1970 jusqu'à une  *)
	(* date et une heure précises Exemple: let ep =                          *)
